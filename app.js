console.log("APP JS UPDATED - NUMERIC SORT VERSION");
...
async function openCategory(category) {
  ...
  if (data) {
    data.sort((a, b) => {
      const aWord = String(a.word ?? '').trim();
      const bWord = String(b.word ?? '').trim();
      const aNum = Number(aWord);
      const bNum = Number(bWord);

      if (!Number.isNaN(aNum) && !Number.isNaN(bNum)) {
        return aNum - bNum;
      }

      return aWord.localeCompare(bWord, undefined, {
        numeric: true,
        sensitivity: 'base'
      });
    });
